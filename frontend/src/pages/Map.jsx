//  Written by Mohammed Shahid and Sri Guhan
...
const Map = () => {
  const [mapCenter, setMapCenter] = useState([50.735, -3.535]); // Default to Streatham
  ...
  useEffect(() => {
    const fetchCampus = async () => {
      try {
        ...
        // Step 1: Get player_id from user_id
        const playerRes = await axios.get(`${API}/api/playerid/${userId}/`);
        const playerId = playerRes.data.player_id;

        // Step 2: Get full player details including campus
        const playerDetails = await axios.get(`${API}/api/player/${playerId}/`);
        const campusId = playerDetails.data.campus;

        // Step 3: Set center based on campus
        if (campusId === 2) {
          setMapCenter([50.72358368964303, -3.5159203681653493]); // St Luke's
        } else {
          setMapCenter([50.735, -3.535]); // Streatham
        }
      } catch (err) {
        console.error("Error getting campus info:", err);
      }
    };
